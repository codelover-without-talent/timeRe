...
def emp_gaussian_xval_processes(processes,par_list,lnd_mk,X,Y,NumFolds):
    pool = mp.Pool(processes = processes)
    results = [pool.apply_async(kdr.emp_gaussian_xval,args=(par,lnd_mk,X,Y,NumFolds)) for par in par_list]
    results = [p.get() for p in results]
    return results
def shrin_gaussian_xval_processes(processes,par_list,lnd_mk,X,Y,NumFolds):
    pool = mp.Pool(processes = processes)
    results = [pool.apply_async(kdr.shrin_gaussian_xval,args = (par,lnd_mk,X,Y,NumFolds)) for par in par_list]
    results = [p.get() for p in results]
    return results
...
emp_xval_results0 = emp_gaussian_xval_processes(processes,par_grid0,lnd_mk_den0,samp_train,label_train,NumFolds_cv)  
emp_error_keys = []
emp_error_vals = []
...
shrin_xval_results0 = shrin_gaussian_xval_processes(processes,par_grid0,lnd_mk_den0,samp_train,label_train,NumFolds_cv)
shrin_error_keys = []
shrin_error_vals = []
...
pickle_out1 = open("sample_data_misr_gaussian","wb")
pickle.dump(data_in_use, pickle_out1)
pickle_out1.close()
       
pickle_out2 = open("lnd_points_misr_gaussian","wb")
pickle.dump(lnd_mk_den0, pickle_out2)
pickle_out2.close()
  
pickle_out3 = open("parameter_cv_misr_gaussian","wb")
